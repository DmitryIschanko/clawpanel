 ...
 * Using /api/v3/tools endpoint with toolkit_versions=latest to get all tools
 */
async function fetchComposioTools(toolkitSlug: string): Promise<ComposioTool[]> {
  ...
  try {
    // Use the correct endpoint with toolkit_versions=latest to get all tools
    // Without this parameter, API returns only base version tools
    ...
    if (response.data && Array.isArray(response.data.items)) {
      return response.data.items.map((item: any) => ({
        ...
        display_name: item.name || item.slug,
...
export async function syncComposioAppTools(appId: number): Promise<void> {
  ...
  try {
    ...
    for (const tool of tools) {
      // Tool slug from API is already in format {TOOLKIT}_{ACTION}
    ...
    const currentSlugs = new Set(tools.map(t => t.name));
