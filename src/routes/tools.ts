...
router.post('/', authenticateToken, requireAdmin, auditLog('create', 'tool'), asyncHandler(async (req, res) => {
  ...
  // Only native tools can be created manually
  const source = 'native';
  ...
  const result = db.prepare(`
    INSERT INTO tools (name, type, source, config, enabled, agent_id)
    VALUES (?, ?, ?, ?, 1, ?)
  `).run(
    name,
    type,
...
router.put('/:id', authenticateToken, requireAdmin, auditLog('update', 'tool'), asyncHandler(async (req, res) => {
  ...
  // For external tools (mcp, composio), only allow updating enabled and agent_id
  const isExternal = tool.source === 'mcp' || tool.source === 'composio';
  
  if (name !== undefined && !isExternal) {
    fields.push('name = ?');
    values.push(name);
  }
  if (config !== undefined && !isExternal) {
    fields.push('config = ?');
