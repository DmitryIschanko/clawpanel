...
router.post('/', authenticateToken, requireAdmin, auditLog('create', 'mcp'), asyncHandler(async (req, res) => {
  ...
  try {
    await syncMcpServerTools(Number(mcpServerId));
  } catch (error) {
    logger.error(`Failed to sync MCP server tools for ${name}:`, error);
  }
...
router.put('/:id', authenticateToken, requireAdmin, auditLog('update', 'mcp'), asyncHandler(async (req, res) => {
  ...
  if (updatedServer.enabled === 1) {
    ...
    // Sync tools to database
    try {
      await syncMcpServerTools(Number(req.params.id));
    } catch (error) {
      logger.error(`Failed to sync MCP server tools for ${updatedServer.name}:`, error);
    }
  } else {
    ...
    try {
      await removeMcpServerTools(Number(req.params.id));
    } catch (error) {
      logger.error(`Failed to remove MCP server tools for ${updatedServer.name}:`, error);
    }
  }
...
router.delete('/:id', authenticateToken, requireAdmin, auditLog('delete', 'mcp'), asyncHandler(async (req, res) => {
  ...
  // Remove tools from database first
  try {
    await removeMcpServerTools(Number(req.params.id));
  } catch (error) {
    logger.error(`Failed to remove MCP server tools for ${server.name}:`, error);
  }
  
  // Remove from mcporter.json
