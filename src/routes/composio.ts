...
import {
  syncComposioAppTools,
  removeComposioAppTools,
} from '../services/composioToolsSync';
...
router.delete('/apps/:id', authenticateToken, requireAdmin, auditLog('delete', 'composio_app'), asyncHandler(async (req, res) => {
  ...
  try {
    await removeComposioAppTools(parseInt(id));
  } catch (error) {
    logger.error('Failed to remove Composio app tools:', error);
  }
...
async function syncToolsForApp(appId: number, apiKey: string): Promise<number> {
  // Sync tools using our new service
  try {
    await syncComposioAppTools(appId);
    return 1; // Return success
  } catch (error) {
    logger.error('Failed to sync Composio app tools:', error);
    return 0;
  }
}
