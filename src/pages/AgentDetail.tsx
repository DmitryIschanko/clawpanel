...
export function AgentDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'prompt' | 'soul' | 'skills' | 'tools'>('general')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {(['general', 'prompt', 'soul', 'skills', 'tools'] as const).map((tab) => (
          <button
            key={tab}
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        {activeTab === 'tools' && !isNew && (
          <AgentTools agentId={Number(id)} />
        )}
      </div>
    </div>
  )
